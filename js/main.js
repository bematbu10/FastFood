            ...
            if (!checkloop) {
                ...
                fetch("http://localhost:3000/api/accounts", {
                    ...
                    .then(data => {
                        ...
                        // Reload danh sách accounts từ DB
                        loadAccountsFromDB();
                    })
                    .catch(err => {
                        toast({ title: 'Lỗi', message: 'Không thể đăng ký!', type: 'error', duration: 3000 });
                        console.error(err);
                    });
            }
...
function logOut() {
    ...
    let vitri = accounts.findIndex(acc => acc.phone == user.phone);
...
function renderOrderProduct() {
    ...
    let arrDonHang = order.filter(o => o.user_id == currentUser.id);

    let orderHtml = "";

    if (arrDonHang.length == 0) {
        orderHtml = `<div class="empty-order-section">
                        <img src="./assets/img/empty-order.jpg">
        ...
        arrDonHang.forEach(item => {
            ...
            chiTietDon.forEach(sp => {
                let infosp = getProductInfo(sp.product_id);
                ...
                <div class="order-history">
                    <div class="order-history-left">
                        ...
                        <div class="order-history-info">
                            ...
                            <p class="order-history-quantity">x${sp.so_luong}</p>
                        </div>
                    </div>
                    <div class="order-history-right">
                        <span class="order-history-current-price">${vnd(sp.don_gia)}</span>
                    </div>
                </div>`;
            });

            let textCompl = item.trang_thai == 1 ? "Đã xử lý" : "Đang xử lý";
            let classCompl = item.trang_thai == 1 ? "complete" : "no-complete";

            productHtml += `
            <div class="order-history-control">
                <div class="order-history-status">
                    <span class="order-history-status-sp ${classCompl}">${textCompl}</span>
                    <button onclick="detailOrder('${item.id}')"><i class="fa-regular fa-eye"></i> Xem chi tiết</button>
                </div>
                <div class="order-history-total">
                    <span class="order-history-total-desc">Tổng tiền: </span>
                    <span class="order-history-toltal-price">${vnd(item.tong_tien)}</span>
                </div>
            </div>
            </div>`;
...
function detailOrder(id) {
    ...
    <span class="detail-order-item-right">
        ${order.thoi_gian_giao ? order.thoi_gian_giao + " - " : ""}${formatDate(order.ngay_giao_hang)}
    </span>
</li>
            ...
            <li class="detail-order-item">
                ...
                <span class="detail-order-item-right">${order.dia_chi_nhan}</span>
            </li>
    ...
    document.querySelector(".detail-order-content").innerHTML = html;
}
...
function showCategory(category) {
    if (!Array.isArray(productAll) || productAll.length === 0) {
        productAll = JSON.parse(localStorage.getItem("products")) || [];
    }
...
async function submitOrder() {
    ...
    let today = new Date(); // ngày giao mặc định
    let giaoGio = document.querySelector("input[name='giaongay']:checked");
    let ngayGiao = today;

    // Nếu chọn giao vào giờ
    if (giaoGio && giaoGio.id === "deliverytime") {
        let selected = document.querySelector(".choise-time").value;
        ngayGiao = new Date();
        // Không cần giờ chi tiết, chỉ lưu ngày
    }

    let order = {
        user_id: user.id,
        tong_tien: getCartTotal(),
        ghi_chu: document.getElementById("ghichu").value || "",
        ngay_dat: new Date(),
        trang_thai: 0,
        hinh_thuc: document.querySelector("input[name='hinhthuc']:checked").value,
        thoi_gian_giao: giaoGio && giaoGio.id === "deliverytime" ? selected : "",
        ngay_giao_hang: ngayGiao, // ⬅️ SỬA TẠI ĐÂY
        dia_chi_nhan: user.address,
        ten_nguoi_nhan: user.fullname,
        sdt_nguoi_nhan: user.phone,
        list_product: user.cart
    };
